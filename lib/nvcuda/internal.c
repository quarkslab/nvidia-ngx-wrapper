...
WINAPI static void unkFunc() {
  puts("call Unknown1 unk func");
  exit(1);
}
...
static BOOL cuda_check_table(const struct cuda_table *orig, struct cuda_table *impl, const char *name)
{
    ...
    {
        FIXME("WARNING: Your CUDA version supports a newer interface for %s than our implementation: got %d funcs, orig has %d funcs \n", name, impl->size/sizeof(void*)-1, orig->size/sizeof(void*)-1);
    }
...
{
    char buffer[128];
    TRACE("(uuid=%s, orig_result=%p)\n", cuda_print_uuid(uuid, buffer, sizeof(buffer)), orig_table);
    ...
    {
      puts("[CUDA] UUID_TlsNotifyInterface not implemented");
      exit(1);
#if 0
