...
{
    const CUresult ret = pcuCtxGetCurrent(pctx);
    TRACE("(*%p = %p) = %d\n", pctx, *pctx, ret);
    return ret;
}
...
{
    const CUresult res = pcuDeviceGetCount(count);
    TRACE("(%p) = (%d, %u)\n", count, *count, res);
    return res;
}
...
{
    const CUresult ret = pcuDeviceGetName(name, len, dev);
    TRACE("(%p, %d, %d) = (%s, %d)\n", name, len, dev, name, ret);
    return ret;
}
...
{
    const CUresult ret = pcuMemcpyHtoDAsync_v2(dstDevice, srcHost, ByteCount, hStream);
    TRACE("(" DEV_PTR ", %p, %lu, %p) = %d\n", dstDevice, srcHost, (SIZE_T)ByteCount, hStream, ret);
    return ret;
}
...
CUresult WINAPI wine_cuModuleLoad(CUmodule *module, const char *fname)
{
    ...
    ret = pcuModuleLoad(module, fname);
    free(unix_name);
    return ret;
}
...
{
    const CUresult ret = pcuStreamCreate(phStream, Flags);
    TRACE("(%p, %u) = %d\n", phStream, Flags, ret);
    return ret;
}
...
{
  CUresult ret = pcuDeviceGetLuid(luid, deviceNodeMask, dev);
  if (ret == 801 /* Not supported */) {
    memset(luid, 0, 8);
    *deviceNodeMask = 0;
    ret = 0;
  }
  TRACE("(%s, %p, %u) = %d\n", luid, deviceNodeMask, dev, ret);
  return ret;
}
...
CUresult WINAPI wine_cuDeviceGetUuid ( CUuuid* uuid, CUdevice dev )
{
  ...
  const CUresult ret = pcuDeviceGetUuid(uuid, dev);
  char uuidhex[33];
  uuidhex[32] = 0;
  for (int i = 0; i < 16; ++i) {
    sprintf(&uuidhex[i*2], "%02X", uuid->bytes[i]);
  }
  TRACE("(%p,%u) = (%s, %d)\n", uuid, dev, uuidhex, ret);
  return ret;
}
