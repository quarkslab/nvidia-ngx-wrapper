...
{
    Mat inImgMat = imread(imagePath, CV_LOAD_IMAGE_COLOR);
    ...
    Mat outImgRgb;
    cvtColor(inImgMat, outImgRgb, CV_BGR2RGB);
...
std::unique_ptr<unsigned char[]> getRgbaImage(const string& imagePath, int& imgWidth, int& imgHeight)
{
    ...
    Mat outImgRgb;
    cvtColor(inImgMat, outImgRgb, CV_BGRA2RGBA);
    if (outImgRgb.data == nullptr)
    {
        return nullptr;
    }
...
bool putRgbaImage(const string& imagePath, unsigned char* imgRgba, int imgWidth, int imgHeight)
{
    ...
    Mat imgBgra;
    cvtColor(img, imgBgra, CV_RGBA2BGRA);
...
bool putRgbImage(const string& imagePath, unsigned char* imgRgb, int imgWidth, int imgHeight)
{
    Mat img = cv::Mat(imgHeight, imgWidth, CV_8UC3, (void*)imgRgb);

    Mat imgBgr;
    cvtColor(img, imgBgr, CV_RGB2BGR);

    imwrite(imagePath, imgBgr);
